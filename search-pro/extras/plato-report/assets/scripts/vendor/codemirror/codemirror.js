...
window.CodeMirror = (function () {
  ...
  function CodeMirror(place, options) {
    ...
    this.view = makeView(
      new BranchChunk([
        new LeafChunk([makeLine("", null, textHeight(display))]),
      ]),
    );
    this.nextOpId = 0;
    loadMode(this);
  ...
  function makeDisplay(place) {
    ...
    d.heightForcer = elt(
      ...
      "\u00a0",
      null,
      "position: absolute; height: " + scrollerCutOff + "px",
    );
    // Will contain the gutters, if any
    d.gutters = elt("div", null, "CodeMirror-gutters");
    d.lineGutter = null;
    // Helper element to properly size the gutter backgrounds
    var scrollerInner = elt(
      "div",
    ...
    d.wrapper = elt(
      "div",
      [d.inputDiv, d.scrollbarH, d.scrollbarV, d.scrollbarFiller, d.scroller],
      "CodeMirror",
    );
  ...
  function maybeUpdateLineNumberWidth(cm) {
    ...
    var doc = cm.view.doc,
      last = lineNumberFor(cm.options, doc.size - 1),
      display = cm.display;
  ...
  function updateDisplayInner(cm, changes, viewPort) {
    ...
    if (changes && maybeUpdateLineNumberWidth(cm)) changes = true;
    display.sizer.style.marginLeft = display.scrollbarH.style.left =
    ...
    if (display.showingTo > to && display.showingTo - to < 20)
      to = Math.min(doc.size, display.showingTo);
  ...
  function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
    ...
    cm.view.doc.iter(from, to, function (line) {
      ...
      if (lineIsHidden(line)) {
        ...
        nextIntact &&
        nextIntact.from <= lineNo &&
        nextIntact.to > lineNo
      ) {
  ...
  function buildLineElement(cm, line, lineNo, dims) {
    ...
    if (
      ...
      !line.bgClass &&
      !line.wrapClass &&
      (!line.widgets || !line.widgets.length)
    )
      return lineElement;
    ...
    if (cm.options.lineNumbers || markers) {
      var gutterWrap = wrap.appendChild(
        elt(
          "div",
          null,
          null,
          "position: absolute; left: " + dims.fixedPos + "px",
        ),
      );
    ...
    if (line.bgClass)
      wrap.appendChild(
        elt("div", "\u00a0", line.bgClass + " CodeMirror-linebackground"),
      );
  ...
  function updateSelectionCursor(cm) {
    var display = cm.display,
      pos = cursorCoords(cm, cm.view.sel.head, "div");
  ...
  function updateSelectionRange(cm) {
    ...
    function drawForLine(line, fromArg, toArg, retTop) {
      ...
      iterateBidiSections(
        ...
        function (from, to, dir) {
          ...
          rVal = retTop
            ? Math.min(rightPos.top, rVal)
            : Math.max(rightPos.bottom, rVal);
    ...
    if (sel.from.line == sel.to.line) {
      ...
      var cur = fromObj,
        merged,
        path = [sel.from.line, sel.from.ch],
        singleLine;
      while ((merged = collapsedSpanAtEnd(cur))) {
        var found = merged.find();
      ...
      if (singleLine) {
        ...
        if (!sel.to.ch)
          ...
          middleBot = drawForLine(
            sel.to.line,
            collapsedSpanAtStart(toObj) ? null : 0,
            sel.to.ch,
            true,
          );
  ...
  function highlightWorker(cm) {
    ...
    doc.iter(
      view.frontier,
      Math.min(doc.size, cm.display.showingTo + 500),
      function (line) {
        if (view.frontier >= cm.display.showingFrom) {
          // Visible
          if (
            highlightLine(cm, line, state) &&
            view.frontier >= cm.display.showingFrom
          ) {
            if (prevChange && prevChange.end == view.frontier) prevChange.end++;
            else
              changed.push(
                (prevChange = { start: view.frontier, end: view.frontier + 1 }),
              );
          }
          ...
          line.stateAfter =
            view.frontier % 5 == 0 ? copyState(view.mode, state) : null;
        }
        ++view.frontier;
        if (+new Date() > end) {
  ...
  function getStateBefore(cm, n) {
    ...
    var pos = findStartLine(cm, n),
      state = pos && getLine(view.doc, pos - 1).stateAfter;
    ...
    view.doc.iter(pos, n, function (line) {
      ...
      var save =
        pos == n - 1 ||
        pos % 5 == 0 ||
        (pos >= view.showingFrom && pos < view.showingTo);
  ...
  function paddingLeft(display) {
    var e = removeChildrenAndAdd(display.measure, elt("pre")).appendChild(
      elt("span", "x"),
    );
    return e.offsetLeft;
  }
  ...
  function coordsChar(cm, x, y) {
    ...
    if (lineNo >= doc.size)
      return { line: doc.size - 1, ch: getLine(doc, doc.size - 1).text.length };
    if (x < 0) x = 0;
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    var fromX = paddingLeft(cm.display),
      toX = getX(to);
  ...
  function endOperation(cm) {
    ...
    if (view.focused && op.updateInput) resetInput(cm, op.userSelChange);

    if (op.textChanged) signal(cm, "change", cm, op.textChanged);
    if (op.selectionChanged) signal(cm, "cursorActivity", cm);
    for (var i = 0; i < op.delayedCallbacks.length; ++i)
      op.delayedCallbacks[i](cm);
  }
  ...
  function readInput(cm) {
    ...
    if (same < prevInput.length)
      ...
      to = {
        line: to.line,
        ch: Math.min(
          getLine(cm.view.doc, to.line).text.length,
    ...
    updateDoc(
      ...
      "end",
      cm.display.pasteIncoming ? "paste" : "input",
      { from: from, to: to },
    );
  ...
  function resetInput(cm, user) {
    ...
    if (!posEq(view.sel.from, view.sel.to)) {
      ...
      minimal =
        hasCopyEvent &&
        (view.sel.to.line - view.sel.from.line > 100 ||
  ...
  function focusInput(cm) {
    if (
      ...
      (ie || document.activeElement != cm.display.input)
    )
  ...
  function mouseEventInWidget(display, e) {
    for (var n = e_target(e); n != display.wrapper; n = n.parentNode)
      if (
        /\bCodeMirror-(?:line)?widget\b/.test(n.className) ||
      ...
      if (type == "double") {
        ...
        if (posLess(cur, startstart))
          extendSelection(
            cm,
            startend,
            clipPos(doc, { line: cur.line, ch: 0 }),
          );
        else
          extendSelection(
            cm,
            startstart,
            clipPos(doc, { line: cur.line + 1, ch: 0 }),
          );
      }
    }
  ...
  function onDrop(e) {
    ...
    if (files && files.length && window.FileReader && window.File) {
      ...
      if (
        cm.view.draggingText &&
        !(posLess(pos, cm.view.sel.from) || posLess(cm.view.sel.to, pos))
      ) {
      ...
      try {
        ...
        if (text) {
          var curFrom = cm.view.sel.from,
            curTo = cm.view.sel.to;
          setSelection(cm, pos, pos);
          if (cm.view.draggingText)
            replaceRange(cm, "", curFrom, curTo, "paste");
  ...
  function clickInGutter(cm, e) {
    ...
    if (mX >= Math.floor(display.gutters.getBoundingClientRect().right))
      return false;
    e_preventDefault(e);
  ...
  function setScrollLeft(cm, val, isScroller) {
    if (
      isScroller
        ? val == cm.view.scrollLeft
        : Math.abs(cm.view.scrollLeft - val) < 2
    )
      return;
  ...
  function onScrollWheel(cm, e) {
    ...
    if (dy && wheelPixelsPerUnit != null) {
      ...
      var top = cm.view.scrollTop,
        bot = top + cm.display.wrapper.clientHeight;
    ...
    if (wheelSamples < 20) {
      if (wheelStartX == null) {
        wheelStartX = scroll.scrollLeft;
        wheelStartY = scroll.scrollTop;
        wheelDX = dx;
        wheelDY = dy;
        setTimeout(function () {
  ...
  function onBlur(cm) {
    if (cm.view.focused) {
      ...
      cm.display.scroller.className = cm.display.scroller.className.replace(
        " CodeMirror-focused",
        "",
      );
    }
  ...
  function updateDoc(cm, from, to, newText, selUpdate, origin) {
    ...
    var split = sawReadOnlySpans && removeReadOnlyRanges(cm.view.doc, from, to);
    if (split) {
      for (var i = split.length - 1; i >= 1; --i)
        updateDocInner(cm, split[i].from, split[i].to, [""], origin);
      if (split.length)
        return updateDocInner(
          cm,
          split[0].from,
          split[0].to,
          newText,
  ...
  function updateDocInner(cm, from, to, newText, selUpdate, origin) {
    ...
    var startSelFrom = view.sel.from,
      startSelTo = view.sel.to;
  ...
  function unredoHelper(cm, type) {
    var doc = cm.view.doc,
      hist = cm.view.history;
    var set = (type == "undo" ? hist.done : hist.undone).pop();
    if (!set) return;
    var anti = {
      events: [],
      fromBefore: set.fromAfter,
      toBefore: set.toAfter,
      fromAfter: set.fromBefore,
    ...
    for (var i = set.events.length - 1; i >= 0; i -= 1) {
      ...
      updateDocNoUndo(
        ...
        { line: end - 1, ch: getLine(doc, end - 1).text.length },
        change.old,
        selPos,
        type,
      );
  ...
  function updateDocNoUndo(cm, from, to, lines, selUpdate, origin) {
    ...
    if (cm.options.lineWrapping) {
      var perLine = Math.max(
        5,
        display.scroller.clientWidth / charWidth(display) - 3,
      );
  ...
  function clipPos(doc, pos) {
    ...
    if (pos.line >= doc.size)
      return { line: doc.size - 1, ch: getLine(doc, doc.size - 1).text.length };
    var ch = pos.ch,
      linelen = getLine(doc, pos.line).text.length;
        ...
        for (var i = 0; i < line.markedSpans.length; ++i) {
          ...
          if (
            ...
            if (posEq(newPos, curPos)) {
              ...
              if (newPos.ch < 0) {
                if (newPos.line)
                  newPos = clipPos(doc, { line: newPos.line - 1 });
                else newPos = null;
              } else if (newPos.ch > line.text.length) {
                if (newPos.line < doc.size - 1)
                  newPos = { line: newPos.line + 1, ch: 0 };
                else newPos = null;
              }
              if (!newPos) {
                if (flipped) {
  ...
  function scrollPosIntoView(cm, pos) {
    for (;;) {
      ...
      var startTop = cm.view.scrollTop,
        startLeft = cm.view.scrollLeft;
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    var curSpaceString = line.text.match(/^\s*/)[0],
      indentation;
    if (how == "smart") {
      indentation = cm.view.mode.indent(
        state,
  ...
  function selectLine(cm, line) {
    extendSelection(
      cm,
      { line: line, ch: 0 },
      clipPos(cm.view.doc, { line: line + 1, ch: 0 }),
    );
  }
  ...
  CodeMirror.prototype = {
    ...
    setValue: operation(null, function (code) {
      var doc = this.view.doc,
        top = { line: 0, ch: 0 },
        lastLen = getLine(doc, doc.size - 1).text.length;
      updateDocInner(
        this,
        top,
        { line: doc.size - 1, ch: lastLen },
        splitLines(code),
        top,
        top,
        "setValue",
    ...
    markText: operation(null, function (from, to, options) {
      return markText(
        this,
        clipPos(this.view.doc, from),
        clipPos(this.view.doc, to),
        options,
        "range",
      );
    }),
    ...
    findMarksAt: function (pos) {
      ...
      pos = clipPos(doc, pos);
      var markers = [],
        spans = getLine(doc, pos.line).markedSpans;
      if (spans)
    ...
    removeLineWidget: operation(null, function (widget) {
      var ws = widget.line.widgets,
        no = lineNo(widget.line);
      if (no == null) return;
      for (var i = 0; i < ws.length; ++i)
        if (ws[i] == widget) ws.splice(i--, 1);
    ...
    setCursor: operation(null, function (line, ch, extend) {
      var pos = clipPos(
        this.view.doc,
        typeof line == "number" ? { line: line, ch: ch || 0 } : line,
      );
    ...
    setLine: operation(null, function (line, text) {
      if (isLine(this.view.doc, line))
        replaceRange(
          ...
          text,
          { line: line, ch: 0 },
          { line: line, ch: getLine(this.view.doc, line).text.length },
        );
    }),
    ...
    removeLine: operation(null, function (line) {
      if (isLine(this.view.doc, line))
        replaceRange(
          ...
          "",
          { line: line, ch: 0 },
          clipPos(this.view.doc, { line: line + 1, ch: 0 }),
        );
    }),
    ...
    moveH: operation(null, function (dir, unit) {
      ...
      if (sel.shift || sel.extend || posEq(sel.from, sel.to))
        pos = findPosH(this, dir, unit, true);
    ...
    deleteH: operation(null, function (dir, unit) {
      ...
      if (!posEq(sel.from, sel.to))
        replaceRange(this, "", sel.from, sel.to, "delete");
      else
        replaceRange(
          this,
          "",
          sel.from,
          findPosH(this, dir, unit, false),
          "delete",
        );
    ...
    moveV: operation(null, function (dir, unit) {
      ...
      var cur = view.sel.head,
        pos = cursorCoords(this, cur, "div");
      var x = pos.left,
        y;
    ...
    scrollTo: function (x, y) {
      if (x != null)
        this.display.scrollbarH.scrollLeft = this.display.scroller.scrollLeft =
          x;
      if (y != null)
        this.display.scrollbarV.scrollTop = this.display.scroller.scrollTop = y;
      updateDisplay(this, []);
    },
  ...
  function lookupKey(name, maps, handle, stop) {
    ...
    for (var i = 0; i < maps.length; ++i) if (lookup(maps[i])) return true;
  }
  ...
  function markedSpansBefore(old, startCh) {
    if (old)
      for (var i = 0, nw; i < old.length; ++i) {
        ...
        if (
          startsBefore ||
          (marker.type == "bookmark" && span.from == startCh)
        ) {
          var endsAfter =
            span.to == null ||
  ...
  function attachMarkedSpans(line, spans) {
    if (!spans) return;
    for (var i = 0; i < spans.length; ++i) spans[i].marker.lines.push(line);
  ...
  function highlightLine(cm, line, state) {
    ...
    var changed = !line.styles,
      pos = 0,
      curText = "",
      curStyle = null;
    var stream = new StringStream(line.text, cm.options.tabSize),
      st = line.styles || (line.styles = []);
    ...
    if (stream.pos > 5000) {
      st[pos++] = line.text.slice(stream.pos);
      st[pos++] = null;
    }
    if (pos != st.length) {
      st.length = pos;
  ...
  function lineContent(cm, realLine, measure) {
    ...
    if (measure && !builder.addedOne)
      measure[0] = builder.pre.appendChild(
        simple ? elt("span", "\u00a0") : zeroWidthElement(cm.display.measure),
      );
  ...
  function buildTokenMeasure(builder, text, style, startStyle, endStyle) {
    for (var i = 0; i < text.length; ++i) {
      ...
      builder.measure[builder.pos++] = buildToken(
        builder,
        text.charAt(i),
        style,
        i == 0 && startStyle,
  ...
  function addChange(
    ...
    if (
      ...
      cur = {
        events: [{ start: start, added: added, old: old }],
      ...
      while (history.done.length > cm.options.undoDepth) history.done.shift();
      if (history.dirtyCounter < 0)
        // The user has made a change after undoing past the last clean state.
        // We can never get back to a clean state now until markClean() is called.
        history.dirtyCounter = NaN;
      else history.dirtyCounter++;
    }
  ...
  function signalLater(cm, emitter, type /*, values...*/) {
    ...
    var args = Array.prototype.slice.call(arguments, 3),
      flist = cm.curOp && cm.curOp.delayedCallbacks;
    function bnd(f) {
      return function () {
        f.apply(null, args);
      };
      ...
      if (part.level % 2 == linedir) {
        if (target < part.from || target > part.to) {
          ...
          target =
            part &&
            (dir > 0 == part.level % 2
              ? moveOneUnit(part.to, -1)
              : moveOneUnit(part.from, 1));
        } else break;
      } else {
