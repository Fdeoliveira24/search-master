  ...
  keyboardManager: {
    ...
    init(searchContainer, searchInput, performSearch) {
      ...
      searchInput.addEventListener(
        "keyup",
        window.tourSearchFunctions.debounce(() => {
...
*/
      ...
      items.forEach((item, index) => {
        try {
          ...
          if (!label) {
            ...
            const shouldInclude =
              (hasSubtitle &&
                window.tourSearchFunctions.config.includeContent
          ...
          if (!label) {
            if (
              data?.subtitle &&
              window.tourSearchFunctions.config.useAsLabel.subtitles
            ) {
    ...
    const performSearch = () => {
      ...
      if (
        searchTerm !== "*" &&
        searchTerm.length < window.tourSearchFunctions.config.minSearchChars
      ) {
        noResults.style.display = "none";
        resultsContainer.classList.remove("visible");
        resultsList.innerHTML = `
      ...
      try {
        ...
        if (searchTerm === "*") {
          ...
          const processedTerm =
            window.tourSearchFunctions.preprocessSearchTerm(searchTerm);
          matches =
        ...
        Object.entries(groupedResults).forEach(([type, results]) => {
          ...
          results.forEach((result) => {
            ...
            resultItem.addEventListener("click", () => {
              ...
              const isMobile =
                window.innerWidth <=
                window.tourSearchFunctions.config.mobileBreakpoint;

              // Check configuration to determine if search should be hidden
              if (
                (isMobile &&
                  window.tourSearchFunctions.config.autoHide.mobile) ||
                (!isMobile &&
                  window.tourSearchFunctions.config.autoHide.desktop)
              ) {
