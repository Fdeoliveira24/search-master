...
(function () {
      ...
      if (
        ...
        const onTourLoaded = function () {
          if (isMainPlayListReady()) {
            ...
            Logger.info(
              "Tour event fired but mainPlayList not ready, falling back to polling",
            );
            startPolling();
          }
        };
      ...
      function startPolling() {
        ...
        const poll = () => {
          ...
          if (Date.now() - start > timeoutMs) {
            clearTimeout(pollTimer);
            Logger.warn(
              "Tour readiness polling timed out after",
              timeoutMs,
              "ms",
            );
            reject(
              new Error("Tour did not become ready within the timeout period"),
            );
            return;
          }
    ...
    return new Promise((resolve, reject) => {
      ...
      function verifyModules() {
        ...
        if (missingModules.length > 0) {
          ...
          reject(
            new Error(`Missing required modules: ${missingModules.join(", ")}`),
          );
          return;
        }
  ...
  async function initializeDependencies() {
    try {
      ...
      await Promise.all([loadFuseJS(), loadDebugTools()]);
  ...
  if (!window._searchProLoaded) {
    ...
    initializeDependencies().then(() => {
      Logger.info(
        "✅ Search Pro dependencies loaded. Will initialize when tour is ready or button is clicked.",
      );
    });
  }
})();
  ...
  function logSearchContainerState(action, show = null) {
    ...
    if (searchContainer) {
      ...
      console.log(
        ...
        searchContainerStyle.visibility,
      );
      console.log(
        "searchContainer background:",
        searchContainerStyle.background,
      );
    ...
    if (resultsContainer) {
      ...
      console.log(
        ...
        resultsContainerStyle.visibility,
      );
      console.log(
        "resultsContainer background:",
        resultsContainerStyle.background,
      );
    ...
    if (resultsSection) {
      ...
      console.log(
        ...
        resultsSection.innerHTML.trim().length > 0,
      );
      console.log(
        "resultsSection child count:",
        resultsSection.childElementCount,
      );
      if (resultsSection.innerHTML.length > 0) {
        console.log(
          "resultsSection first 100 chars:",
        ...
        for (const mutation of mutationsList) {
          if (mutation.type === "childList") {
            console.log("🔍 DOM childList mutation in search container:", {
              ...
              target:
                ...
                (mutation.target.className
                  ? "." + mutation.target.className.split(" ").join(".")
                  : ""),
            });
            ...
            if (
              ...
              console.log(
                `🔍 ${mutation.attributeName} changed for ${target.tagName + (target.id ? "#" + target.id : "")}`,
                {
...
function ensureAPIExposed() {
  ...
  if (window.SearchProModules) {
    ...
    if (
      ...
      window.tourSearchFunctions.cleanup =
        window.SearchProModules.Lifecycle.cleanup;
    }

    // Add diagnostic function to help with debugging
    if (
      !window.tourSearchFunctions.getSearchState &&
      window.SearchProModules.Lifecycle
    ) {
      window.tourSearchFunctions.getSearchState =
        window.SearchProModules.Lifecycle.getState;
    }
  }
}
