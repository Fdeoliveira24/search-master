...
window.tourDebug = (function () {
  ...
  function getElementType(overlay) {
    ...
    if (className === "HotspotPanoramaOverlay") {
      ...
      if (data.hasOpenableMedia && !data.hasPanoramaAction)
        return "Hotspot-VideoLauncher";
      return "Hotspot";
    }
  ...
  function inspectBasicV1() {
    ...
    try {
      ...
      if (!playlists || playlists.length === 0) {
        ...
        playlists.forEach((pl, pIdx) => {
          ...
          items.forEach((item, idx) => {
            ...
            const subtitle =
              media.get("subtitle") || media.get("data")?.subtitle;
      ...
      const directMethods = Object.keys(player).filter(
        (x) => typeof player[x] === "function",
      );
      const allMethods = Array.from(
        new Set([...protoMethods, ...directMethods]),
      );
          ...
          if (media.get("class") === "Panorama") {
            ...
            overlays.forEach((ov) => {
              ...
              if (filters.logOnlyMediaHotspots && attr.hasOpenableMedia) {
                console.warn(
                  `[v5] Media Hotspot: ${attr.label || "Unlabeled"}`,
                  attr,
                );
              }
            });
...
(function () {
  ...
  function initializeDebugger() {
    console.log(
      "%c3DVista Tour Debugger",
      "font-size:20px; color:#3b82f6; font-weight:bold;",
    );
    console.log(
      "%cThis script analyzes the tour object structure",
      "color:#64748b;",
    );

    if (!window.tour || !window.tour.player) {
      console.error(
        "Tour or tour.player not available yet. Try running debugTour() manually later.",
      );
      ...
      if (!items || !items.length) {
        ...
        for (let i = 0; i < Math.min(3, items.length); i++) {
          ...
          try {
            ...
            const mediaProps = [];
            for (let j = 0; j < 20; j++) {
              // Try some common property indexes
              try {
            ...
            try {
              ...
              if (overlays.length > 0) {
                ...
                try {
                  console.log("overlay.get('data'):", overlay.get("data"));
                } catch (e) {}
    ...
    console.log(
      "%cDebug utilities available at window.tourDebug",
      "color:green; font-weight:bold;",
    );
    console.log(
      "Try tourDebug.getMediaData() or tourDebug.getCurrentOverlays()",
    );
  }
})();
