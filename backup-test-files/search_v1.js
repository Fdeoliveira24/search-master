  ...
  function _getElementType(overlay, label) {
    ...
    try {
      ...
      if (overlay.class) {
        ...
        // 3D Model and Sprite types - MISSING FROM ORIGINAL
        if (className === "SpriteModel3DObject") return "3DModel";
        if (className === "Model3DObject") return "3DModel";
        if (className === "Sprite3DObject") return "3DHotspot";
        if (className === "SpriteHotspotObject") return "3DHotspot";

        // Audio types
        if (className === "AudioPanoramaOverlay") return "Audio";
        if (className === "SoundObject") return "Audio";

        // Additional overlay types
        if (className === "ProjectedImagePanoramaOverlay")
          return "ProjectedImage";
        if (className === "PolygonPanoramaOverlay") return "Polygon";
        if (className === "MapPanoramaOverlay") return "Map";
        if (className === "PhotoAlbumPanoramaOverlay") return "PhotoAlbum";
        ...
        if (className === "HotspotPanoramaOverlay") {
          ...
          if (overlay.data) {
            ...
            if (overlay.data.is3DModel) return "3DModel";
          }
          ...
          if (overlayLabel.includes("3d") || overlayLabel.includes("model"))
            return "3DModel";
        ...
        try {
          ...
          if (className === "HotspotPanoramaOverlay") {
            ...
            try {
              ...
              if (data.is3DModel) return "3DModel";
            } catch (e) {}
      ...
      if (
        ...
        return "Polygon";
      if (overlay.model3D || (overlay.data && overlay.data.model3D))
        return "3DModel";
      if (
        overlay.audio ||
        overlay.sound ||
        (overlay.data && (overlay.data.audio || overlay.data.sound))
      )
        return "Audio";
      ...
      if (overlay.model) return "3DModel";
      if (overlay.sprite3D) return "3DHotspot";

      // QUATERNARY: Use label patterns for "3D-Model-Hotspot"
      const overlayLabel = (overlay.label || label || "").toLowerCase();
      if (overlayLabel) {
        ...
        if (overlayLabel.includes("polygon")) return "Polygon";
        if (overlayLabel.includes("audio") || overlayLabel.includes("sound"))
          return "Audio";
        // ADDED: 3D Model detection patterns
        if (
          overlayLabel.includes("3d-model") ||
          overlayLabel.includes("3d_model")
        )
          return "3DModel";
        if (
          overlayLabel.includes("model-hotspot") ||
          overlayLabel.includes("model_hotspot")
        )
          return "3DModel";
        if (overlayLabel.includes("3d") && overlayLabel.includes("model"))
          return "3DModel";
        if (overlayLabel.includes("sprite") && overlayLabel.includes("3d"))
          return "3DHotspot";
        if (overlayLabel.includes("goto") || overlayLabel.includes("info"))
          return "Hotspot";
      }
    ...
    const attemptTrigger = () => {
      try {
        ...
        if (retryCount < config.maxRetries) {
          console.log(
            `Element trigger attempt ${retryCount} failed, retrying in ${config.retryInterval}ms...`,
          );
        ...
        items.forEach((item, index) => {
          try {
            ...
            try {
              ...
              console.warn(
                `Error getting data for item at index ${index}: ${e.message}`,
              );
              data = {};
            }
            ...
            if (!label && !subtitle && !hasTags) {
              ...
              if (
                ...
                allowedMediaIndexes.length > 0
              ) {
                if (!allowedMediaIndexes.includes(index)) return;
              }
              if (
                filterMode === "blacklist" &&
                blacklistedMediaIndexes.length > 0
              ) {
            ...
            if (!label) {
              ...
              const shouldInclude =
                (hasSubtitle &&
                  _config.includeContent.unlabeledWithSubtitles) ||
                (hasTags && _config.includeContent.unlabeledWithTags) ||
                (!hasSubtitle &&
                  !hasTags &&
                  _config.includeContent.completelyBlank);
            ...
            try {
              ...
              if (
                ...
                try {
                  const allOverlays =
                    tour.player.getByClassName("PanoramaOverlay");
                  if (Array.isArray(allOverlays)) {
                    ...
                    overlays = allOverlays.filter((overlay) => {
                      try {
                        ...
                        return (
                          parentMedia &&
                          parentMedia.get("id") === media.get("id")
                        );
                      } catch (e) {
                        return false;
                      }
              ...
              overlays.forEach((overlay, overlayIndex) => {
                try {
                  ...
                  if (
                    !overlayLabel &&
                    _config.includeContent.elements.skipEmptyLabels
                  )
                    return;
                  ...
                  fuseData.push({
                    type: elementType,
                    label: elementLabel,
                    tags: Array.isArray(overlayData.tags)
                      ? overlayData.tags
                      : [],
                    parentIndex: index,
              ...
              console.warn(
                `Error accessing overlays for panorama at index ${index}:`,
                overlaysError,
              );
            }
          } catch (error) {
    ...
    const performSearch = () => {
      ...
      try {
        ...
        if (searchTerm === "*") {
          ...
          matches =
            typeof processedTerm === "string" && processedTerm.startsWith("=")
