...
(function () {
  ...
  function loadCSS() {
    return new Promise((resolve) => {
      ...
      if (document.querySelector('link[href="search-pro/search-01.css"]')) {
        resolve();
        return;
      }
      ...
      cssLink.href = "search-pro/search-01.css";
...
window.tourSearchFunctions = (function () {
  ...
  function _safelyGetProperty(obj, path, defaultValue = null) {
    try {
      ...
      Logger.debug(`Error accessing property ${path}:`, e);
      return defaultValue;
    }
  }
  ...
  return {
    ...
    searchHistory: {
      ...
      clear: function () {
        ...
        // In the _initializeSearch function, after the SearchResults component definition:

        // Initialize the search results component
        const searchResults = SearchResults.init(_elements.container, _config);

        // Update the performSearch function to use the searchResults component
        const performSearch = () => {
          const searchInput = _elements.container.querySelector("#tourSearch");
          const searchTerm = searchInput ? searchInput.value.trim() : "";
          const clearButton =
          ...
          if (!searchTerm) {
            searchResults.clear();
            _elements.container.classList.remove("has-results");
            _elements.container
              .querySelector(".search-results")
              .classList.remove("visible");
            _elements.container.querySelector(".search-results").style.display =
              "none";
            _elements.container.querySelector(".results-section").innerHTML =
              renderSearchHistory();
            return;
          }
          ...
          if (
            searchTerm !== "*" &&
            searchTerm.length < _config.minSearchChars
          ) {
            searchResults.clear();
            _elements.container.classList.remove("has-results");
            _elements.container
        ...
        `;
          ...
          // Don't show results container yet, wait until we have results
          ...
          try {
            ...
            if (!matches || !matches.length) {
              searchResults.clear();
              _elements.container.classList.remove("has-results");
              _elements.container.querySelector(".no-results").style.display =
                "flex";
              return;
            }

            // Now that we have results, show the container and add the has-results class
            _elements.container.classList.add("has-results");
            resultsContainer.style.display = "block";
            setTimeout(() => resultsContainer.classList.add("visible"), 10);
            ...
            // Render results using the SearchResults component
            searchResults.render(matches, searchTerm, highlightMatch);
          } catch (error) {
            Logger.error("Search error:", error);
            // Show error message in results
            _elements.container.querySelector(".results-section").innerHTML = `
        ...
        // Update the keyboard manager to work with the new SearchResults component
        const keyboardManager = {
          init(searchContainer, searchInput, performSearch) {
            if (!searchContainer || !searchInput) {
              Logger.error("Invalid parameters for keyboard manager");
            ...
            // Update the selected item in the results
            const updateSelection = (newIndex) => {
              searchResults.setSelectedIndex(newIndex);
            };
            ...
            const handleKeyDown = (e) => {
              ...
              const resultItems = searchResults.getResultItems();
              const currentIndex = searchResults.selectedIndex;
              ...
              switch (e.key) {
                ...
                case "ArrowUp":
                  ...
                  if (currentIndex > 0) {
                    ...
                    searchInput.focus();
                    searchResults.setSelectedIndex(-1);
                  }
                  break;
        ...
        // Update the event binding to use the new keyboard manager
        _bindSearchEventListeners(
          _elements.container,
          _elements.container.querySelector("#tourSearch"),
          _elements.container.querySelector(".clear-button"),
