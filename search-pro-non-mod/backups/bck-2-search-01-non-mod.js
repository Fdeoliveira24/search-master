...
// [1.0.2] Cross-window communication channel (BroadcastChannel API)
...
// [1.0.3] Cleanup for search event listeners
...
// [1.0.4] Shared selection index for search results
let selectedIndex = -1;
...
window.tourSearchFunctions = (function () {
  ...
  class ConfigBuilder {
    ...
    setThumbnailSettings(options) {
      ...
      // **ADD: Validate showFor configuration consistency**
      const validTypes = [
        "panorama",
        "hotspot",
        "polygon",
      ...
      // **KEEP: Resolve pixel size based on named size (IMPORTANT - DON'T REMOVE)**
    ...
    // [4.2.0.13] Set animation options
  ...
  function _applySearchStyling() {
    ...
    // Set mobile override attributes for CSS targeting
    searchContainer.setAttribute(
      "data-focus-mode",
      mobileOverrides.focusMode ? "true" : "false",
    );
    searchContainer.setAttribute(
      "data-fullscreen-results",
      mobileOverrides.fullScreenResults ? "true" : "false",
    );
    searchContainer.setAttribute(
    ...
    root.style.setProperty(
      ...
      Math.min(thumbnailRadius, 20) + "px",
    );
    ...
    // [4.10.1.9.3] Set search field styles
    const animConfig = _config.animations || {};
    root.style.setProperty(
      "--animation-enabled",
      animConfig.enabled ? "1" : "0",
    );
    root.style.setProperty(
    ...
    root.style.setProperty(
      "--animation-searchbar-open",
      (animConfig.searchBar?.openDuration || 300) + "ms",
    );
    root.style.setProperty(
      "--animation-searchbar-close",
      (animConfig.searchBar?.closeDuration || 200) + "ms",
    );
    root.style.setProperty(
      "--animation-results-fade",
      (animConfig.results?.fadeInDuration || 200) + "ms",
    );
    root.style.setProperty(
      "--animation-results-slide",
      (animConfig.results?.slideDistance || 10) + "px",
    );
    root.style.setProperty(
      "--animation-results-stagger",
      (animConfig.results?.staggerDelay || 50) + "ms",
    );

    // [4.10.1.9.4] Set reduced motion styles
    if (
      animConfig.reducedMotion?.respectPreference &&
      window.matchMedia("(prefers-reduced-motion: reduce)").matches
    ) {
      root.style.setProperty(
        "--animation-duration-fast",
        (animConfig.reducedMotion?.fallbackDuration || 100) + "ms",
      );
      root.style.setProperty(
        "--animation-duration-normal",
        (animConfig.reducedMotion?.fallbackDuration || 100) + "ms",
  ...
  function _initializeSearch(tour) {
    ...
    function _shouldIncludePanorama(
      ...
      // [6.11.0.5] NEW - Apply unique item name filtering (exact match)
      if (label) {
        const uniqueNameFilterMode = _config.filter.uniqueNames?.mode;
        if (
          uniqueNameFilterMode === "whitelist" &&
        ...
        Object.entries(groupedResults)
          ...
          .forEach(([type, results]) => {
            ...
            results.forEach((result) => {
              ...
              // [6.19.1.17.6.13] Determine thumbnail visibility with consistent type checking
              ...
              try {
                ...
                // **FIX: Handle special cases for consistent naming**
                const typeMapping = {
                  "3dmodel": "3dmodel",
                  "3dhotspot": "3dhotspot",
                ...
                elementTypeLower =
                  typeMapping[elementTypeLower] || elementTypeLower;

                // Fallback for invalid/empty types
                if (!elementTypeLower || elementTypeLower.length === 0) {
...
`;
              ...
              if (imgElement) {
                imgElement.handleImageError = function () {
                  ...
                  const iconType =
                    this.closest(".result-item")?.dataset.type || "default";

                  console.log("Image load failed for:", originalSrc);
                  console.log("Element type:", iconType);

                  // Skip format switching and go directly to default images
                  if (!triedFormats.includes("default")) {
                    ...
                    // Try type-specific default first, then fallback to generic default
                    let defaultPath;

                    // Log what we're trying to load
                    console.log(
                      "Looking for default image for type:",
                      iconType,
                    );
                    console.log(
                      "Available defaults:",
                      Object.keys(_config.thumbnailSettings.defaultImages),
                    );
                    ...
                    if (
                      ...
                      defaultPath =
                        _config.thumbnailSettings.defaultImages[iconType];
                      console.log("Found type-specific default:", defaultPath);
                    } else {
                      defaultPath =
                        _config.thumbnailSettings.defaultImages.default;
                      console.log("Using generic default:", defaultPath);
                    }

                    // Clean and normalize the path
                    defaultPath = defaultPath.replace(/^\.\//, "");
                    const defaultImage = `${baseUrl}/${defaultPath}`;
                    console.log("Loading default image from:", defaultImage);

                    this.src = defaultImage;
                    return;
                  }

                  // If even default image fails, hide the image
                  console.error("All image formats failed to load");
                  this.classList.add("hidden");
                };

                // Add this function to your code for debugging
                function debugImagePaths() {
                  ...
                  Object.entries(
                    config.thumbnailSettings.defaultImages,
                  ).forEach(([type, path]) => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setTimeout(function () {
    ...
    //  ***THIS IS WHERE END USERS START TO MAKE CHANGES***
    ...
    if (window.searchFunctions) {
      window.searchFunctions.updateConfig({
        ...
        businessData: {
          ...
          replaceTourData: true, // *** true/false - Replace tour labels with business labels
          includeStandaloneEntries: true, // *** true/false - Add business entries without tour matches
          businessDataFile: "business.json",
          businessDataDir: "business-data",
          matchField: "id",
          businessDataUrl: `${window.location.origin}${window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/"))}/search-pro-non-mod/business-data/business.json`,
        },
        // [9.1.3.1] Google Sheets support
        // ==========================================
        // GOOGLE SHEETS / LOCAL CSV INTEGRATION
        // ==========================================
        ...
        // [9.1.3.3] Thumbnail support
        thumbnailSettings: {
          enableThumbnails: true, // *** true or false - Enable custom thumbnails
          ...
          // [9.1.3.4] Thumbnail appearance settings
          alignment: "left",
        ...
        // [9.1.3.5] Element inclusion options
        includeContent: {
          ...
          elements: {
            includePanoramas: true, // Include panoramas in search results (ADDED)
        ...
        // [9.1.3.6] Elementfiltering options
        filter: {
          mode: "none", // "none", "whitelist", "blacklist"
          allowedValues: [], // Values to allow if mode is "whitelist"
          blacklistedValues: [], // Values to block if mode is "blacklist"
          allowedMediaIndexes: [], // Media indexes to allow
          blacklistedMediaIndexes: [], // Media indexes to block
          elementTypes: {
            mode: "none", // "none", "whitelist", "blacklist"
          ...
          // *** FOR END USERS: Filter by exact item names ***
          uniqueNames: {
            mode: "blacklist", // "none", "whitelist", "blacklist"
            allowedNames: [], // Exact item names to allow (e.g., ["Lobby", "Room-101", "Main-Entrance"])
            blacklistedNames: [], // Exact item names to block (e.g., ["Hidden-Room", "Private-Area"])
          },
        },

        // [9.1.3.7] ENHANCED: Add updated display labels
        displayLabels: {
          Panorama: "Panorama",
          Hotspot: "Hotspot",
          Polygon: "Polygon",
        ...
        // [9.1.3.8] Add appearance settings
        appearance: {
          searchField: {
            borderRadius: {
              topLeft: 35,
          ...
          // [9.1.3.9] Add animations settings
          animations: {
            enabled: true, // *** true or false - Enable/disable all animations
            duration: {
              fast: 200, // *** Fast animations (ms) - buttons, hovers
              normal: 300, // *** Normal animations (ms) - search open/close
              slow: 500, // *** Slow animations (ms) - complex transitions
            },
            easing: "cubic-bezier(0.22, 1, 0.36, 1)", // *** Animation easing function
            searchBar: {
              openDuration: 300, // *** Search bar open animation (ms)
              closeDuration: 200, // *** Search bar close animation (ms)
              scaleEffect: true, // *** true or false - Scale effect on open
            },
            results: {
              fadeInDuration: 200, // *** Results fade in time (ms)
              slideDistance: 10, // *** Slide animation distance (px)
              staggerDelay: 50, // *** Delay between result items (ms)
            },
            reducedMotion: {
              respectPreference: true, // *** Respect user's motion preferences
              fallbackDuration: 100, // *** Reduced animation time (ms)
            },
          },

          // [9.1.3.10] Add tag color customizations
          colors: {
            ...
            // [9.1.3.11] Add tag color customizations
            ...
            tagHover: "#bae6fd", // *** Lighter blue on hover
          },

          // [9.1.3.12] Add tag appearance settings
          tags: {
        ...
        // [9.1.3.6] Add search bar settings
        searchBar: {
          ...
          mobileOverrides: {
            ...
            visibility: {
              ...
              showOnScroll: false, // Show when scrolling
              hideThreshold: 100, // Hide when scrolling past this threshold
            },
            focusMode: false, // Darkens background when search is active
            fullScreenResults: false, // Show results full screen on mobile
          },
        },

        // [9.1.3.7] Apply display settings to search results
        display: {
        ...
        // [9.1.3.8] - Label Fallback Options
        useAsLabel: {
      ...
      // Add this right after your updateConfig call
      console.log(
        "Thumbnail settings:",
        window.searchFunctions.getConfig().thumbnailSettings,
      );

      // Add this debugging function and call it
      function debugImagePaths() {
        const config = window.searchFunctions.getConfig();
        const baseUrl =
          window.location.origin +
